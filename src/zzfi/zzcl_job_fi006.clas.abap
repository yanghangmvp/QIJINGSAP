CLASS zzcl_job_fi006 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZZCL_JOB_FI006 IMPLEMENTATION.
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    DATA: lv_begin TYPE i,
          ...
          lv_curr  TYPE i,
          lv_times TYPE i.
    DATA: lt_tmp TYPE TABLE OF zztta006.
    ...
    SELECT *
      FROM zztta006
      INTO TABLE @DATA(lt_ta006).

    "删除已有主键
    READ TABLE lt_ta006 INTO DATA(ls_ta006) INDEX 1.
    IF sy-subrc = 0.
      lv_sql_delete = |DELETE FROM { ls_conf-zztable } | &&
                      |WHERE ENT_COD = '{ ls_ta006-ent_cod }' AND | &&
                      |YEAR = '{ ls_ta006-zzyear }' AND | &&
                      |MONTH = '{ ls_ta006-zzmonth }';|.
      "推送数据
      zzcl_jdbc_ta_process=>send_ta( lv_sql_delete ).
    ENDIF.

    lv_sql_head = |INSERT INTO { ls_conf-zztable }| &&
                  |(YEAR, MONTH, GP_ENT_COD, GP_ENT_DES, ENT_COD| &&
                  |, ENT_DES, GP_ACC_COD, GP_ACC_DES, BR_ACC_COD, BR_ACC_DES| &&
                  |, BR_CTP_COD, BR_CTP_DES, GP_CTP_COD, GP_CTP_DES, TD| &&
                  |, CURRENCY, AMOUNT, SYS_ID, DATEUPD) values|.
    ...
    TRY.
        ...
        DO lv_times TIMES.
          ...
          CLEAR: lt_tmp.
          APPEND LINES OF lt_ta006 FROM lv_begin TO lv_end TO lt_tmp.
