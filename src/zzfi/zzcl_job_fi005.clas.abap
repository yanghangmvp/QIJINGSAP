CLASS zzcl_job_fi005 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZZCL_JOB_FI005 IMPLEMENTATION.
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    DATA: lv_begin TYPE i,
          ...
          lv_curr  TYPE i,
          lv_times TYPE i.
    DATA: lt_tmp TYPE TABLE OF zztta005.
    ...
    SELECT *
      FROM zztta005
      INTO TABLE @DATA(lt_ta005).

    "删除已有主键
    READ TABLE lt_ta005 INTO DATA(ls_ta005) INDEX 1.
    IF sy-subrc = 0.
      lv_sql_delete = |DELETE FROM { ls_conf-zztable } | &&
                      |WHERE BR_ENT_COD = '{ ls_ta005-br_ent_cod }' AND | &&
                      |YEAR = '{ ls_ta005-zzyear }' AND | &&
                      |MONTH = '{ ls_ta005-zzmonth }';|.
      "推送数据
    ...
    lv_sql_head = |INSERT INTO { ls_conf-zztable }| &&
                  ...
                  |, BR_ENT_DES, GP_ACC_COD, GP_ACC_DES, BR_ACC_COD, BR_ACC_DES| &&
                  |, CF_COD, CF_DES, CURRENCY, DEBITBL, CREDITBL| &&
                  |, DEBITBL_A, CREDITBL_A, CURRENCY_T, DEBITBL_T, CREDITBL_T| &&
                  |, DEBITBL_A_T, CREDITBL_A_T, SYS_ID, DATEUPD) values|.
    ...
    TRY.
        ...
        DO lv_times TIMES.
          ...
          CLEAR: lt_tmp.
          APPEND LINES OF lt_ta005 FROM lv_begin TO lv_end TO lt_tmp.
