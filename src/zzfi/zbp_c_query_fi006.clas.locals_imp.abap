CLASS lhc_fi006 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR fi006 RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR fi006 RESULT result.

    METHODS read FOR READ
      IMPORTING keys FOR READ fi006 RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK fi006.

    METHODS zpush FOR MODIFY
      IMPORTING keys FOR ACTION fi006~zpush RESULT result.
...
CLASS lhc_fi006 IMPLEMENTATION.
  ...
  METHOD zpush.
    ...
    DATA: ls_zztta005 TYPE zztta005,
          lt_zztta005 TYPE TABLE OF zztta005.

    DATA: lr_fi006 TYPE REF TO zzcl_query_fi006.
    ...
    "获取数据
    CREATE OBJECT lr_fi006.
    CALL METHOD lr_fi006->read_data
      EXPORTING
    ...
    lt_zztta005 = CORRESPONDING #( lt_result ).

    INSERT zztta005 FROM TABLE @lt_zztta005.

    DATA job_template_name TYPE cl_apj_rt_api=>ty_template_name VALUE 'ZZ_JT_TA005'.
...
CLASS lsc_zc_query_fi006 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zc_query_fi006 IMPLEMENTATION.
