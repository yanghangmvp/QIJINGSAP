CLASS lhc_fi002 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_features FOR INSTANCE FEATURES
      IMPORTING keys REQUEST requested_features FOR fi002 RESULT result.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR fi002 RESULT result.

    METHODS read FOR READ
      IMPORTING keys FOR READ fi002 RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK fi002.

    METHODS zpush FOR MODIFY
      IMPORTING keys FOR ACTION fi002~zpush RESULT result.

ENDCLASS.

CLASS lhc_fi002 IMPLEMENTATION.
  ...
  METHOD zpush.
    ...
    DATA: ls_zztta001 TYPE zztta001,
          lt_zztta001 TYPE TABLE OF zztta001.

    DATA: lr_fi002 TYPE REF TO zzcl_query_fi002.
    ...
    "获取数据
    CREATE OBJECT lr_fi002.
    CALL METHOD lr_fi002->read_data
      EXPORTING
        it_filters = lt_filters
      IMPORTING
        et_result  = lt_result.

    lt_zztta001 = CORRESPONDING #( lt_result ).

    INSERT zztta001 FROM TABLE @lt_zztta001.

    DATA job_template_name TYPE cl_apj_rt_api=>ty_template_name VALUE 'ZZ_JT_TA001'.
    ...
    job_parameter-t_value = VALUE #(
        ( low = key-%key-uuid sign = 'I' option = 'EQ' )
     ).
...
CLASS lsc_zc_query_fi002 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zc_query_fi002 IMPLEMENTATION.
