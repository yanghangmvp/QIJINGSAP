CLASS lhc_fi007 DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR fi007 RESULT result.

    METHODS read FOR READ
      IMPORTING keys FOR READ fi007 RESULT result.

    METHODS lock FOR LOCK
      IMPORTING keys FOR LOCK fi007.

    METHODS zpush FOR MODIFY
      IMPORTING keys FOR ACTION fi007~zpush RESULT result.

ENDCLASS.

CLASS lhc_fi007 IMPLEMENTATION.
  ...
  METHOD zpush.
    ...
    DATA: ls_zztta006 TYPE zztta006,
          lt_zztta006 TYPE TABLE OF zztta006.

    DATA: lr_fi007 TYPE REF TO zzcl_query_fi007.
    ...
    "获取数据
    CREATE OBJECT lr_fi007.
    CALL METHOD lr_fi007->read_data
      EXPORTING
        it_filters = lt_filters
      IMPORTING
        et_result  = lt_result.

    lt_zztta006 = CORRESPONDING #( lt_result ).

    INSERT zztta006 FROM TABLE @lt_zztta006.

   DATA job_template_name TYPE cl_apj_rt_api=>ty_template_name VALUE 'ZZ_JT_TA006'.
...
CLASS lsc_zc_query_fi007 DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
...
CLASS lsc_zc_query_fi007 IMPLEMENTATION.
