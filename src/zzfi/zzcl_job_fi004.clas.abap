CLASS zzcl_job_fi004 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZZCL_JOB_FI004 IMPLEMENTATION.
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    DATA: lv_begin TYPE i,
          ...
          lv_curr  TYPE i,
          lv_times TYPE i.
    DATA: lt_tmp TYPE TABLE OF zztta004.
    ...
    SELECT *
      FROM zztta004
      INTO TABLE @DATA(lt_ta004).

    "删除已有主键
    READ TABLE lt_ta004 INTO DATA(ls_ta004) INDEX 1.
    IF sy-subrc = 0.
      lv_sql_delete = |DELETE FROM { ls_conf-zztable } | &&
                      |WHERE BR_ENT_COD = '{ ls_ta004-br_ent_cod }' AND | &&
                      |YEAR = '{ ls_ta004-zzyear }' AND | &&
                      |MONTH = '{ ls_ta004-zzmonth }';|.
      "推送数据
      zzcl_jdbc_ta_process=>send_ta( lv_sql_delete ).
    ENDIF.

    lv_sql_head = |INSERT INTO { ls_conf-zztable }| &&
                  |(YEAR, MONTH, GP_ENT_COD, GP_ENT_DES, BR_ENT_COD| &&
                  |, BR_ENT_DES, GP_ACC_COD, GP_ACC_DES, BR_ACC_COD, BR_ACC_DES, BR_ACC_AREA| &&
                  |, CURRENCY, BEGINBL_Y, BEGINBL, DEBITBL, CREDITBL| &&
                  |, DEBITBL_A, CREDITBL_A, ENDBL, CURRENCY_T, BEGINBL_Y_T| &&
                  |, BEGINBL_T, DEBITBL_T, CREDITBL_T, DEBITBL_A_T, CREDITBL_A_T| &&
                  |, ENDBL_T, SYS_ID, DATEUPD) values|.
    ...
    TRY.
        ...
        DO lv_times TIMES.
          ...
          CLEAR: lt_tmp.
          APPEND LINES OF lt_ta004 FROM lv_begin TO lv_end TO lt_tmp.
