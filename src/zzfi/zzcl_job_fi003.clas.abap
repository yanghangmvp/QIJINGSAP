CLASS zzcl_job_fi003 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZZCL_JOB_FI003 IMPLEMENTATION.
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    DATA: lv_begin TYPE i,
          ...
          lv_curr  TYPE i,
          lv_times TYPE i.
    DATA: lt_tmp TYPE TABLE OF zztta003.
    ...
    SELECT *
      FROM zztta003
      INTO TABLE @DATA(lt_ta003).

    "删除已有主键
    READ TABLE lt_ta003 INTO DATA(ls_ta003) INDEX 1.
    IF sy-subrc = 0.
      lv_sql_delete = |DELETE FROM { ls_conf-zztable } | &&
                      |WHERE ENT_COD = '{ ls_ta003-ent_cod }' AND | &&
                      |YEAR = '{ ls_ta003-zzyear }' AND | &&
                      |MONTH = '{ ls_ta003-zzmonth }';|.
      "推送数据
    ...
    lv_sql_head = |INSERT INTO { ls_conf-zztable }| &&
                  ...
                  |,ENT_DES, VOUCHER, LINE, TYPE, PROJ_COD| &&
                  |,PROJ_NAME, SUB_COD, SUB_DES, AMOUNT, NOTE| &&
                  |,SYS_ID, DATEUPD) values|.
    ...
    TRY.
        ...
        DO lv_times TIMES.
          ...
          CLEAR: lt_tmp.
          APPEND LINES OF lt_ta003 FROM lv_begin TO lv_end TO lt_tmp.
