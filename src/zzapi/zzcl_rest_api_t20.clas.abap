CLASS zzcl_rest_api_t20 DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    METHODS get_fingercode
      RETURNING VALUE(ev_fingercode) TYPE string.

    METHODS get_encrypt
      IMPORTING
                iv_data           TYPE string
      RETURNING VALUE(ev_encrypt) TYPE string.
...
CLASS ZZCL_REST_API_T20 IMPLEMENTATION.
...
*    ENDCASE.
  ...
  METHOD get_fingercode.
    TRY.
        cl_abap_message_digest=>calculate_hash_for_char(
          EXPORTING
             if_algorithm     = 'MD5'
             if_data          = |{ me->zzif_rest_api~ms_conf-zztkurl }{ me->zzif_rest_api~ms_conf-zzctid }|
           IMPORTING
            ef_hashstring    = DATA(lv_fingercode)
               ).

        ev_fingercode = to_lower( lv_fingercode ).
  ...
  METHOD get_encrypt.
    ...
    lv_input_xs = /ui2/cl_json=>string_to_raw(
     EXPORTING iv_string   = iv_data
     iv_encoding = '4110' ).

*    lv_key_xs = /ui2/cl_json=>string_to_raw(
*     EXPORTING iv_string   = CONV string( me->zzif_rest_api~ms_conf-zzctsecret )
*     iv_encoding = '4110' ).
*
*    lv_key_xs16 = lv_key_xs.
    lv_aes_key = me->zzif_rest_api~ms_conf-zzctsecret..
