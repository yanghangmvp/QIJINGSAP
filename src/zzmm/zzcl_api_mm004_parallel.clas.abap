CLASS zzcl_api_mm004_parallel DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    DATA: gv_productdescription TYPE i_productdescription_2-productdescription.
...
CLASS zzcl_api_mm004_parallel IMPLEMENTATION.
  ...
  METHOD if_abap_parallel~do.
    ...
    CLEAR: gv_productdescription.
    SELECT SINGLE
           b~productdescription
      FROM i_product WITH PRIVILEGED ACCESS AS a
      JOIN i_productdescription_2 WITH PRIVILEGED ACCESS AS b ON a~product = b~product
     WHERE a~productexternalid = @gs_entry-partcode
       AND b~language = 'E'
      INTO @gv_productdescription.

    SELECT COUNT(*)
      FROM i_product WITH PRIVILEGED ACCESS
     WHERE productexternalid = @gs_entry-partcode.
...
*&---确定http 状态
        ...
        IF status-code = '201'.
          ...
          gs_resp-sapnum = ls_entry-partcode.
        ELSE.
...
*&---确定http 状态
    ...
    IF ls_entry-partendesc IS NOT INITIAL.
      IF gv_productdescription IS INITIAL.

        DATA: ls_cdescription TYPE ty_description.

        CLEAR: gs_http_req, gs_http_resp, ls_description, ls_cdescription.

        lv_productdescription = ls_entry-partendesc.
        ls_cdescription-product = ls_entry-partcode.
        ls_cdescription-language = 'EN'.
        ls_cdescription-productdescription = lv_productdescription.

        gs_http_req-version = 'ODATAV2'.
        gs_http_req-method = 'POST'.
        gs_http_req-url = |/API_PRODUCT_SRV/A_Product('{ ls_entry-partcode }')/to_Description?sap-language=zh|.

        "传入数据转JSON
        gs_http_req-body = /ui2/cl_json=>serialize(
              data          = ls_cdescription
              compress      = abap_true
              name_mappings = gt_mapping ).
        ...
        IF gs_http_resp-code <> '201'.
          ...
          LOOP AT ls_rese-error-innererror-errordetails INTO ls_errordetails WHERE severity = 'error'.
            gs_resp-msgtx = gs_resp-msgtx && '/' && ls_errordetails-message.
          ENDLOOP.
          IF gs_resp-msgtx IS INITIAL.
            gs_resp-msgtx = gs_resp-msgtx && '/' && ls_rese-error-message-value.
          ENDIF.
        ENDIF.
