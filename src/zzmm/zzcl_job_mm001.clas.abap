...
CLASS ZZCL_JOB_MM001 IMPLEMENTATION.
  ...
  METHOD if_apj_rt_exec_object~execute.
    ...
    DATA: lr_lifnr TYPE RANGE OF i_supplier-supplier.
    ...
    DATA: lv_job(1).
    DATA: lv_numb TYPE zzt_rest_append-zznumb VALUE 'DMS002'.
    ...
    LOOP AT it_parameters INTO DATA(ls_param).
      CASE ls_param-selname.
        ...
        WHEN 'LIFNR'.
          APPEND VALUE #( low = ls_param-low high = ls_param-high sign = ls_param-sign option = ls_param-option ) TO lr_lifnr.
        WHEN 'JOB'.
          IF ls_param-low = 'X'.
            lv_job = abap_true.
          ENDIF.
    ...
    SELECT a~*
      FROM zc_bp_timst AS a
      JOIN i_supplier AS b ON a~businesspartner = b~supplier
     WHERE businesspartner IN @lr_lifnr
    ...
    DATA: lv_begin TYPE i,
          ...
          lv_pack  TYPE i,
          lv_times TYPE i.
    DATA: lt_tmp TYPE zzt_mmi010_in.
    ...
    TRY.
        ...
        DO lv_times TIMES.
          ...
          CLEAR: lt_tmp.
          APPEND LINES OF lt_bp FROM lv_begin TO lv_end TO lt_tmp.

          "推送数据
          o_resp = lr_mm010->push( lt_tmp ).
