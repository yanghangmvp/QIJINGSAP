CLASS zzcl_api_mm005 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zzcl_api_mm005 IMPLEMENTATION.


  METHOD inbound.

    DATA: l_in_tab TYPE cl_abap_parallel=>t_in_inst_tab,
          l_inst   TYPE REF TO zzcl_api_mm005_parallel.
...
*    lt_mtco = VALUE #( FOR bill IN ls_req-interface-bill
*                         ( mtco = bill-billheader-mtco )
*                     ).
*
*    IF lt_mtco IS NOT INITIAL.
*      SELECT a~productexternalid
*        FROM i_product WITH PRIVILEGED ACCESS AS a
*        JOIN @lt_mtco AS b ON a~productexternalid = b~mtco
*        INTO TABLE @DATA(lt_product).
*      SORT lt_product BY productexternalid.
*    ENDIF.

    LOOP AT ls_req-interface-bill INTO DATA(ls_bill).
      APPEND NEW zzcl_api_mm005_parallel( ls_bill-billheader ) TO l_in_tab.

*      CLEAR: ls_resp.
*      READ TABLE lt_product INTO DATA(ls_product) WITH KEY productexternalid = ls_bill-billheader-mtco BINARY SEARCH.
*      IF sy-subrc <> 0.
*        me->zzcreate(
*          EXPORTING
*              i_req = ls_bill-billheader
*          IMPORTING
*              o_resp = ls_resp
*        ).
*
...
*      ENDIF.
*
*      ls_resp-uuid = ls_bill-billheader-zguid.
*
...
*&---确定http 状态
        ...
        IF status-code = '201'.
          o_resp-msgty  = 'S'.
          o_resp-msgtx  = 'success'.
          o_resp-sapnum = ls_billheader-mtco.
        ELSE.
